    ...
    public partial class frmRegistrarVenta : Form
    {
        ...
        private void cboTipoPlan_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if(idTipoPlan == 1)
            {
                ...
                fnOcultarParaRenovacion(dgvPagoPlan, lblPagoPlan, true);
        ...
        private void cboPlan_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
             ...
             fnDevolverPrecioTarifa(idPlan, Convert.ToInt32(cboTipoVenta.SelectedValue));
            //fnCargarTablaTarifa();
        ...
        private Boolean fnDevolverPrecioTarifa(Int32 idPlan, Int32 tipoCon)
        {
            ...
            try
            {
                clsTipTarifa = objBlTipTarifa.blDevolverPrecios(idPlan,tipoCon);
        ...
        private void txtDocumentoC_TextChanged(object sender, EventArgs e)
        {
            ...
            if (numCaracNroDocumento >= 3)
            {
                ...
                if (Convert.ToInt32(cboTipoVenta.SelectedValue) == 2)
                {
                    bResul = fnBuscarCliente(txtDocumentoC, 0, -4, dgDocumentoC, tipoCon.lnTipoConC,cboTipoClienteC,cboTipoDocumentoC);
                }
                else
                {
                    bResul = fnBuscarCliente(txtDocumentoC, 0, -3, dgDocumentoC, tipoCon.lnTipoConC,cboTipoClienteC,cboTipoDocumentoC);
                }
                if (!bResul)
                {
        ...
        private void txtDocumentoRP_TextChanged(object sender, EventArgs e)
        {
            ...
            var result = FunValidaciones.fnValidarTexboxs(lstValidacionRespPago[2].textbox, erDocumentoRP, imgDocumentoRP, true, true, true, maxCaracteres, maxCaracteres, maxCaracteres, maxCaracteres, "Ingrese correctamente");
            lstValidacionRespPago[2].estado = result.Item1;
            lstValidacionRespPago[2].mensaje = result.Item2;
            Int32 numCaracNroDocumento = txtDocumentoRP.TextLength;
            if (numCaracNroDocumento >= 3)
            {
                Boolean bResul;

                bResul = fnBuscarCliente(txtDocumentoRP, 0, -1, dgDocumentoRP, tipoCon.lnTipoConRP, cboTipoClienteRP, cboTipoDocumentoRP);
                if (!bResul)
                {
                    MessageBox.Show("Error al Buscar Vehiculo. Comunicar a Administrador de Sistema", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
        ...
        private void rdbSi_CheckedChanged(object sender, EventArgs e)
        {
            ...
            int idCliente = Convert.ToInt32(cboTipoDocumentoC.SelectedValue ?? 0);
        ...
        private Boolean fnPreguntaQuienPaga(object sender, EventArgs e)
        {
            ...
            if (rdbSi.Checked == true)
            {
                ...
                //gbDinamico.Location = new Point(18, 320);
            ...
            else
            {
                ...
                //gbDinamico.Location = new Point(8, 513);
                //tabControl1.Height = 722 + 87;
            }
            return true;
        }
        ...
        private void fnCalcularTotalPago()
        {
            ...
            {
                lstDV[i].Cantidad = lstVehiculo.Count;
        ...
        private List<DetalleVenta> fnGenerarPagoPrincipal()
        {
            ...
            Int32 cuota = 0;
            if (Convert.ToInt32(cboTipoVenta.SelectedValue)==1)
            {
                if (idTipoPlan == 1)
                {
                    rentaAdelantada = 0;
            ...
            else if(Convert.ToInt32(cboTipoVenta.SelectedValue) == 2)
            {
                cuota = 1;
                if (idTipoPlan == 1)
                {
            ...
            else if (Convert.ToInt32(cboTipoVenta.SelectedValue) == 3)
            {
                if (idTipoPlan == 1)
                {
                    arrayPrecio = new double[]
            ...
            if (lnTipoCon==-1)
            {
                ...
                {
                    lstDetV = lstPP;
                    if (lstPP.Count == arrayPrimerPago.Count())
                    {
                        for (int i = 0; i < lstPP.Count; i++)
                        {
                            double precioUnitario = fnConvertirPrecioMoneda(clsMonedaTarifa.idMoneda, arrayPrecio[i], clsMoneda.idMoneda);

                            double precioDescontado = fnConvertirPrecioMoneda(clsMonedaTarifa.idMoneda, arrayDescuentoPrecio[i], clsMoneda.idMoneda);
                            int cantidad = lstVehiculo.Count;
                            double importe = (precioUnitario * cantidad) - precioDescontado;

                            lstDetV[i].Numeracion = i + 1;
                            lstDetV[i].Descripcion = arrayPrimerPago[i];
                            lstDetV[i].idTipoTarifa = clsTarifa.IdTarifa;
                            lstDetV[i].PrecioUni = precioUnitario;
                            lstDetV[i].Descuento = arrayDescuentoCantidad[i];
                            lstDetV[i].gananciaRedondeo = arrayGananciaPro[i];
            ...
            else
            {
                for (int i = 0; i <= arrayPrimerPago.Count() - 1; i++)
                {
                    ...
                    lstDetV.Add(new DetalleVenta
                    {
                        ...
                        Descuento = arrayDescuentoCantidad[i],
                        gananciaRedondeo = arrayGananciaPro[i],
        ...
        private void irActualizar_Click(object sender, EventArgs e)
        {
            ...
            for (Int32 i = 0; i < lstPP.Count; i++)
            {
                if (ClsVentaGeneral.clsDetalleVentaCabecera.lstDetalleVenta.Count>1)
                {
                    ...
                    }else if (lstPP[i].Descripcion == CadenaRecortada)
                    {
                            lstPP.Remove(lstPP[i]);
                    }
                }
